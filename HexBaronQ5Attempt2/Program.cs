...
namespace HexBaronCS
{
    ...
    class HexGrid
    {
        ...
        private string CreateOddLineIndices(ref int listPositionOfTile)
        {
            string line = "";
            for (var count = 1; count <= size / 2; count++)
            {
                if (count > 1 & count < size / 2)
                {
                    line += listPositionOfTile % 10 + @"\__/";
                    listPositionOfTile += 1;
                    line += listPositionOfTile / 10;
                }
                else if (count == 1)
                {
                    line += @" \__/" + listPositionOfTile / 10;
                }
            }
            line += listPositionOfTile % 10 + @"\__/";
            listPositionOfTile += 1;
            if (listPositionOfTile < tiles.Count())
            {
                line += (listPositionOfTile / 10).ToString() + (listPositionOfTile % 10).ToString() + @"\" + Environment.NewLine;
            }
            else
            {
                line += @"\" + Environment.NewLine;
        ...
        private string CreateEvenLineIndices(bool firstEvenLine, ref int listPositionOfTile)
        {
            string line = " /" + listPositionOfTile / 10;
            for (var count = 1; count <= size / 2 - 1; count++)
            {
                line += listPositionOfTile % 10;
                listPositionOfTile += 1;
            ...
            if (firstEvenLine)
            {
                line += listPositionOfTile % 10 + @"\__" + Environment.NewLine;
            }
            else
            {
                line += listPositionOfTile % 10 + @"\__/" + Environment.NewLine;
        ...
        public string GetGridAsIndices(bool p1Turn)
        {
            int listPositionOfTile = 0;
            player1Turn = p1Turn;
            string gridAsString = CreateTopLine() + CreateEvenLineIndices(true, ref listPositionOfTile);
            listPositionOfTile += 1;
            gridAsString += CreateOddLineIndices(ref listPositionOfTile);
            for (var count = 1; count <= size - 2; count += 2)
            {
                listPositionOfTile += 1;
                gridAsString += CreateEvenLineIndices(false, ref listPositionOfTile);
                listPositionOfTile += 1;
                gridAsString += CreateOddLineIndices(ref listPositionOfTile);
            }
            return gridAsString + CreateBottomLine();
