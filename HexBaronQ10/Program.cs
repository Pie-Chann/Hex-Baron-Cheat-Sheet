        ...
        public static void PlayGame(Player player1, Player player2, HexGrid grid)
        {
            ...
            do
            {
                ...
                for (int count = 1; count <= 3; count++)
                {
                    ...
                    string ToRead = Console.ReadLine().ToLower();
                    string[] splitter = ToRead.Split(' ');
                    if (splitter[0] == "salvage")
                    {
                        if (count == 1)
                        {
                            commands.Add(ToRead);
                            Console.WriteLine("salvage added");
                        }
    ...
    class HexGrid
    {
                                     ...
                                     int piecesInSupply)
        {
            switch (items[0])
            {
                ...
                case "salvage":
                    {
                        int lumberGained = ExecuteSalvageCommand(items);
                        lumberChange += lumberGained;
                        if (lumberGained == 5)
                        {
